...
import { RPC_NODES, QUERY_EPOCH_GROUP_DATA_PATH, API_BASE_URL } from "./config.js";
import { getRpcClient, resetRpcClient } from "./rpc-client.js";
import {
  getLatestEpochInfo,
  getEpochStartBlockHeight,
  decodeQueryGetEpochGroupDataResponse,
  getEpochGroupDataRest,
  getEpochGroupDataByStoreKey,
  epochGroupDataStoreKeyHex,
} from "./epoch.js";
import { getBlock, getValidatorsWithProofContext } from "./block.js";
import { abciQueryWithProof, verifyProofAgainstRoot } from "./merkle.js";
import { verifyCommitFromValidators } from "./commit-verify.js";
import { encodeEpochGroupDataRequest } from "./epoch.js";

/** Parse optional epoch ID from env EPOCH_ID or CLI --epoch <id>. */
function getEpochIdArg(): number | undefined {
...
async function main(): Promise<void> {
  ...
  try {
    ...
    let latestEpochId: number;
    let newEpochStartHeight: number;

    if (epochIdArg !== undefined) {
      console.log("\n--- Epoch by ID ---");
      latestEpochId = epochIdArg;
      newEpochStartHeight = await getEpochStartBlockHeight(latestEpochId);
      ...
      latestEpochId = epochInfo.latestEpoch.index;
      newEpochStartHeight = epochInfo.latestEpoch.pocStartBlockHeight;
    ...
    let storeKeyResult = await getEpochGroupDataByStoreKey(latestEpochId, "", newEpochStartHeight);
    ...
    if (storeKeyResult.decoded) {
      console.log(
        "Decoded: epochIndex=",
        storeKeyResult.decoded.epochIndex,
        "pocStartBlockHeight=",
        storeKeyResult.decoded.pocStartBlockHeight,
        "validation_weights (participants):",
        storeKeyResult.decoded.validationWeights.length
      );
    } else {
    ...
    if (
      storeKeyResult.value.length > 0 &&
      storeKeyResult.proofOps &&
      storeKeyResult.proofOps.length > 0
    ) {
      try {
        const proofBlock = await getBlock(storeKeyResult.height);
        const header = proofBlock.block.header as { appHash?: Uint8Array; app_hash?: string };
        const appHash =
          header.appHash ??
        ...
        if (appHash && appHash.length > 0) {
          ...
          if (process.env.DEBUG_VERIFY === "1" || process.env.DEBUG_VERIFY === "true") {
            console.log("EpochData store proof: block height", storeKeyResult.height, "app_hash from", header.appHash != null ? "header.appHash (Uint8Array)" : "header.app_hash (base64->Uint8Array)", "length", appHash.length);
          }
          const valid = verifyProofAgainstRoot(
            storeKeyResult.proofOps,
            keyBytes,
            storeKeyResult.value,
            appHash
          );
        ...
        console.warn("EpochData store proof verification error:", (e as Error).message);
      }
    ...
    if (API_BASE_URL) {
      ...
      const appHash = (newEpochBlock.block.header as { appHash?: Uint8Array }).appHash;

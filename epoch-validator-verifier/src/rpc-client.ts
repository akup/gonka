...
let currentClient: Tendermint34Client | null = null;
 ...
 */
export async function getRpcClient(): Promise<{ client: Tendermint34Client; url: string }> {
  if (currentClient != null && currentUrl != null) {
    return { client: currentClient, url: currentUrl };
  }

  const errors: Array<{ url: string; err: Error; detail: string }> = [];
  for (const url of RPC_NODES) {
    try {
      ...
      if (cause) console.warn("  cause:", cause.message);
    }
  }

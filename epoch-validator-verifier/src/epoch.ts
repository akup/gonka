...
import {
  QUERY_EPOCH_INFO_PATH,
  QUERY_EPOCH_GROUP_DATA_PATH,
  EPOCH_INFO_REQUEST_B64,
  API_BASE_URL,
  EPOCH_GROUP_DATA_REST_PATH,
  INFERENCE_STORE_PATH,
} from "./config.js";
import { abciQueryWithProof, type ProofOp } from "./merkle.js";
...
export interface EpochGroupDataRestResponse {
  epoch_group_data?: {
    ...
    epoch_policy?: string;
    validation_weights?: Array<{ member_address?: string; weight?: string }>;
  };
}
 ...
 */
export async function getEpochGroupDataRest(
  epochIndex: number,
  modelId: string,
  blockHeight?: number
): Promise<EpochGroupDataDecoded | null> {
  ...
  if (blockHeight != null) headers[X_COSMOS_BLOCK_HEIGHT] = String(blockHeight);
  const res = await fetch(url, { method: "GET", headers });
  if (!res.ok) return null;
  const json = (await res.json()) as EpochGroupDataRestResponse;
  const raw = json.epoch_group_data;
  if (!raw) return null;
  const validationWeights: ValidationWeightDecoded[] = (raw.validation_weights ?? []).map((v) => ({
 ...
 */
export function epochGroupDataStoreKeyHex(epochIndex: number, modelId: string): string {
  const buf: number[] = [EPOCH_GROUP_DATA_PREFIX];
  const n = Number(epochIndex);
  if (n < 0 || !Number.isInteger(n) || n > Number.MAX_SAFE_INTEGER) throw new Error("epochIndex must be a safe integer");
  ...
  for (let i = 0; i < modelBytes.length; i++) buf.push(modelBytes[i]);
 ...
 */
export function decodeEpochGroupDataFromStore(bytes: Uint8Array): EpochGroupDataDecoded {
 ...
 */
export async function getEpochGroupDataByStoreKey(
  epochIndex: number,
  modelId: string,
  height?: number
): Promise<EpochGroupDataByStoreKeyResult> {
  ...
  let decoded: EpochGroupDataDecoded | null = null;
 ...
 */
export async function getLatestEpochInfo(): Promise<EpochInfoResult> {
  ...
  try {
    ...
    throw new Error(
      `EpochInfo abci_query failed (path=${QUERY_EPOCH_INFO_PATH}): ${err.message}` +
        (code ? ` code=${code}` : "") +
 ...
 */
export async function getEpochStartBlockHeight(epochIndex: number): Promise<number> {
  const { client } = await getRpcClient();
  ...
  if (res.code !== 0) {
    throw new Error(
      `EpochGroupData query failed for epoch ${epochIndex}: code=${res.code} log=${res.log}`
    );
  }
...
function decodeEpochInfoResponse(bytes: Uint8Array): {
  blockHeight: number;
  ...
  while (offset < bytes.length) {
    ...
    if (wireType === 0) {
      ...
      offset += c;
      if (fieldNum === 1) blockHeight = Number(value);
    } else if (wireType === 2) {
...
function decodeEpochSubmessage(bytes: Uint8Array): { index: number; pocStartBlockHeight: number } {
  let offset = 0;
  let index = 0;
  let pocStartBlockHeight = 0;
  while (offset < bytes.length) {
    ...
    if (wireType === 0) {
      const { value, consumed: c } = readVarint(bytes, offset);
      offset += c;
      if (fieldNum === 1) index = Number(value);
      else if (fieldNum === 2) pocStartBlockHeight = Number(value);
    } else if (wireType === 2) {
 ...
 */
export function decodeQueryGetEpochGroupDataResponse(bytes: Uint8Array): EpochGroupDataDecoded {
...
function extractEpochGroupDataBytes(responseBytes: Uint8Array): Uint8Array | null {
  let offset = 0;
...
function decodeEpochGroupData(bytes: Uint8Array): EpochGroupDataDecoded {
  let offset = 0;
  let pocStartBlockHeight = 0;
  let epochIndex = 0;
  const validationWeights: ValidationWeightDecoded[] = [];
  let field2: number | undefined;
  let field3Bytes: Uint8Array | undefined;
  ...
  while (offset < bytes.length) {
    ...
    if (wireType === 0) {
      const { value, consumed: c } = readVarint(bytes, offset);
      offset += c;
      if (fieldNum === 1) pocStartBlockHeight = Number(value);
      else if (fieldNum === 2) field2 = Number(value);
      else if (fieldNum === 16) epochIndex = Number(value);
    } else if (wireType === 2) {
      const { value: len, consumed: lc } = readVarint(bytes, offset);
      offset += lc;
      const sub = bytes.subarray(offset, offset + Number(len));
  ...
  const out: EpochGroupDataDecoded = { pocStartBlockHeight, epochIndex, validationWeights };
  if (field2 !== undefined) out.field2 = field2;
  if (field3Bytes?.length) out.field3Bytes = field3Bytes;
  return out;
}

function decodeValidationWeight(bytes: Uint8Array): ValidationWeightDecoded {
  let offset = 0;
  let memberAddress = "";
  let weight = 0;
  while (offset < bytes.length) {
    ...
    if (wireType === 0) {
      const { value, consumed: c } = readVarint(bytes, offset);
      offset += c;
      if (fieldNum === 2) weight = Number(value);
    } else if (wireType === 2) {
      const { value: len, consumed: lc } = readVarint(bytes, offset);
      offset += lc;
      const sub = bytes.subarray(offset, offset + Number(len));
      offset += Number(len);
      if (fieldNum === 1) memberAddress = new TextDecoder().decode(sub);
    }
  }
 ...
 */
export function debugEpochGroupDataFields(responseBytes: Uint8Array): Array<{ fieldNum: number; wireType: number; len?: number }> {
  const inner = extractEpochGroupDataBytes(responseBytes);
  if (!inner || inner.length === 0) return [];
  const out: Array<{ fieldNum: number; wireType: number; len?: number }> = [];
  let offset = 0;
...
function decodeEpochGroupDataPocStartHeight(bytes: Uint8Array): number {
  const inner = extractEpochGroupDataBytes(bytes);
  if (!inner) return 0;
...
function readTag(buf: Uint8Array, offset: number): { fieldNum: number; wireType: number; consumed: number } {
...
function readVarint(buf: Uint8Array, offset: number): { value: bigint; consumed: number } {
  let value = BigInt(0);
  let shift = 0;
  let consumed = 0;
...
export function encodeEpochGroupDataRequest(epochIndex: number, modelId: string): Uint8Array {
...
function encodeVarint(fieldNum: number, value: bigint): Uint8Array {
...
function varintEncode(n: bigint): Uint8Array {
  const bytes: number[] = [];
  let x = n;
  while (x > BigInt(0x7f)) {
